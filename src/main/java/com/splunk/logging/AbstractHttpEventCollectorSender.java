...
/**
 * This is an helper class that sends logging events to Splunk http event collector.
 */
public abstract class AbstractHttpEventCollectorSender extends TimerTask implements HttpEventCollectorMiddleware.IHttpSender {
     ...
     * @param metadata events metadataDict
     */
    public AbstractHttpEventCollectorSender(
    ...
    public synchronized void send(final String severity, final String message) {
        ...
        HttpEventCollectorEventInfo.LoggerEvent event=new HttpEventCollectorEventInfo.LoggerEvent(severity,message);
        //meta information is recreated in case metadataDict got updated outside of Sender
        HttpEventCollectorEventInfo eventInfo =
                new HttpEventCollectorEventInfo(getMetaData(), event);
        send(eventInfo);
    }
     ...
     * @note in case of batching the event isn't sent immediately
     */
    public synchronized void send(HttpEventCollectorEventInfo eventInfo) {
        //add it to the batch
        eventsBatch.add(eventInfo);
        eventsBatchSize += eventInfo.getSize();
    ...
    private HttpEventCollectorEventInfo.MetaData getMetaData(){
        // event metadataDict
        String index = metadataDict.get(MetadataIndexTag);
        String source = metadataDict.get(MetadataSourceTag);
        String sourceType = metadataDict.get(MetadataSourceTypeTag);
        HttpEventCollectorEventInfo.MetaData metaData=new HttpEventCollectorEventInfo.MetaData(index,source,sourceType,null);
        return metaData;
    }
     ...
     *
     * @param eventInfo
     * @return json string to send to splunk
     */
    public abstract String serializeEventInfo(HttpEventCollectorEventInfo eventInfo);
    ...
    public void stopHttpClient() throws SecurityException {
        if (httpClient != null) {
            try {
                httpClient.close();
            } catch (IOException e) {}
